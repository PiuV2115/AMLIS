# xgboost_employee_attrition_balanced.py
...
# 1. Load dataset
# ==========================
df = pd.read_csv(r"D:\attrition\WA_Fn-UseC_-HR-Employee-Attrition-WithID.csv")  # dataset with EmployeeID

# Target variable
y = df["Attrition"].map({"Yes": 1, "No": 0})
X = df.drop(columns=["Attrition"])

# Optional: Drop EmployeeID from features for training
feature_cols = [col for col in X.columns if col != "EmployeeID"]

# Identify categorical & numerical columns
categorical_cols = X[feature_cols].select_dtypes(include=["object"]).columns.tolist()
...
xgb_clf = xgb.XGBClassifier(
    ...
    scale_pos_weight=1,  # We will use SMOTE, no need to scale pos weight
...
joblib.dump(pipeline, "models/xgb_attrition_model_new.pkl")
